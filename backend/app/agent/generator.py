...
12. For cross-BU questions, use feature.customer_cross_bu_feature first when its retrieved
   ...
   When breakdown_plan is present it is authoritative. Project exactly expected_columns,
   use only its registered dimensions/metrics/features, and do not add or remove a group.
   A physical_column plan must GROUP BY exactly the registered physical dimension columns.
    ...
    def generate(self, request: GenerationRequest) -> GenerationResponse:
        payload = self.client.complete_json(
            ...
            json.dumps({"question": request.question, "intent": request.route.intent.value,
                        ...
                        "join_plan": request.join_plan,
                        "breakdown_plan": (
                            request.breakdown_plan.model_dump(mode="json")
                            if request.breakdown_plan else None
                        )}, ensure_ascii=False),
        )
